...
`)}}const z$=!0;class A$ extends C${constructor(t,r){super(t,{skip_prompt:!0,skip_special_tokens:!0}),this.cb=r}on_finalized_text(t){this.cb(t)}}class P$ extends bn{constructor(){super(),this.interrupted=!1}interrupt(){this.interrupted=!0}reset(){this.interrupted=!1}_call(t,r){return new Array(t.length).fill(this.interrupted)}}const zn=new P$;async function O$(){try{return(await navigator.gpu.requestAdapter()).features.has("shader-f16")}catch{return!1}}class Gt{static async getInstance(t=null){return this.model_id??(this.model_id=await O$()?"Xenova/Phi-3-mini-4k-instruct_fp16":"Xenova/Phi-3-mini-4k-instruct"),this.tokenizer??(this.tokenizer=Rp.from_pretrained(this.model_id,{legacy:!0,progress_callback:t})),this.model??(this.model=Xh.from_pretrained(this.model_id,{dtype:"q4",device:"webgpu",use_external_data_format:!0,progress_callback:t})),Promise.all([this.tokenizer,this.model])}}T(Gt,"model_id",null),T(Gt,"model",null),T(Gt,"tokenizer",null),T(Gt,"streamer",null);async function B$(e){const[t,r]=await Gt.getInstance(),n=t.apply_chat_template(e,{add_generation_prompt:!0,return_dict:!0});let s,i=0;const a=d=>{s??(s=performance.now());let c;i++>0&&(c=i/(performance.now()-s)*1e3),self.postMessage({status:"update",output:d,tps:c,numTokens:i})},o=new A$(t,a);self.postMessage({status:"start"});const l=await r.generate({...n,max_new_tokens:512,streamer:o,stopping_criteria:zn}),u=t.batch_decode(l,{skip_special_tokens:!1});self.postMessage({status:"complete",output:u})}async function R$(){self.postMessage({status:"loading",data:"Loading model"});let e="/";location.href.toLowerCase().indexOf("github.io")>-1||z$?(e="/web-ai-showcase/",pe.allowLocalModels=!1,pe.allowRemoteModels=!0):(pe.localModelPath="/models/",pe.allowLocalModels=!0,pe.allowRemoteModels=!1),pe.backends.onnx.wasm.wasmPaths=`${e}models/wasm/ort-web@1_19_0_dev/`;const[t,r]=await Gt.getInstance(s=>{self.postMessage(s)});self.postMessage({status:"loading",data:"Compiling shaders and warming up model..."});const n=t("a");await r.generate({...n,max_new_tokens:1}),self.postMessage({status:"ready"})}self.addEventListener("message",async e=>{const{type:t,data:r}=e.data;switch(t){case"load":R$();break;case"generate":zn.reset(),B$(r);break;case"interrupt":zn.interrupt();break;case"reset":zn.reset();break}})})();
